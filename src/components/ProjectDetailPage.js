...
const ProjectDetailPage = ({ project, onBack }) => {
  if (!project) {
    return (
      <div className="text-center text-gray-400 py-16">
        Project not found.
        <button onClick={onBack} className="flex items-center gap-2 mt-4 mx-auto text-orange-400 hover:text-orange-500 transition-colors duration-200">
  ...
  return (
    <div className="bg-gray-800 p-8 rounded-xl shadow-lg border border-gray-700 transition-all duration-300">
      <button onClick={onBack} className="flex items-center gap-2 mb-8 text-orange-400 hover:text-orange-500 transition-colors duration-200">
        <ArrowLeft size={16} /> Back to Projects
      </button>
      
      {/* The layout is now a vertical stack on all screen sizes */}
      <div className="flex flex-col space-y-8">
        {/* Project Video and Links */}
        <div className="flex-shrink-0 w-full">
          {videoId ? (
            <div className="w-full rounded-lg overflow-hidden border border-gray-700 max-w-4xl mx-auto relative" style={{ paddingBottom: '56.25%' }}>
              <iframe
          ...
          <div className="mt-8 flex justify-center md:justify-start gap-4">
            <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 text-gray-300 hover:text-orange-500 rounded-full border border-gray-600 hover:border-orange-500 font-medium transition-colors duration-200">
        ...
        <div className="flex-grow w-full">
          ...
          <div className="mt-6">
            ...
            {project.overview.map((paragraph, index) => (
              <p key={index} className="text-lg text-gray-300 mt-4">{paragraph}</p>
            ))}
          </div>

          {/* Technologies Used */}
          <div className="mt-6">
            <h3 className="text-2xl font-bold text-gray-200">Technologies Used</h3>
          ...
          <div className="mt-6">
            ...
            <ul className="list-disc list-inside space-y-4 mt-4 text-gray-300">
              {project.futureGoals.map((goal, index) => (
                <li key={index}>
                  <h5 className="font-semibold text-xl text-gray-200 mb-1">{goal.title}</h5>
